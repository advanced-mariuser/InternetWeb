...
void UdpSocket::SetRecvTimeout(int seconds, int microseconds)
{
    struct timeval tv;
    tv.tv_sec = seconds;
...
void UdpSocket::SendTo(const std::string& data, const std::string& ip, uint16_t port)
{
    ...
    if (sendto(m_fd, data.c_str(), data.length(), 0, (struct sockaddr*)&destAddr, sizeof(destAddr)) < 0)
    {
        throw std::runtime_error("sendto() failed: " + GetSocketErrorString());
    }
}

std::optional<std::string> UdpSocket::RecvFrom(sockaddr_in& sourceAddr)
{
