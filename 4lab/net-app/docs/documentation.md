...
*   **`Socket.h / Socket.cpp`**
    ...
    *   **Особенность:** Использует семантику перемещения (`move semantics`) для безопасной передачи владения сокетом (например, от слушающего сокета сервера к потоку-обработчику клиента).
...
*   **`Protocol.h`**
    ...
    *   **Особенность:** Является header-only для простоты включения. Использует функции `htonl`/`ntohl` для корректной работы с сетевым порядком байт, что делает протокол платформонезависимым.
